...
# Version 2.1 - 2 Feb 2025
#   Make script capable of getting variables from main, prepare to make pip installable
...
# * Some updates based off research by Mark Woan (@woanware) - https://github.com/woanware/javaidx/tree/master/Documents
...
def sec2_parse(data, meta):
    ...
    print(f"IP: {data_IP}")
    if csv:
        csv_body = f"{file_name}|{data_URL}|{data_IP}"
    for _ in range(0, sec2_fields):
        ...
        if csv:
            csv_body += f"|{value}"
    if csv:
        csvfile = f"{file_name}.csv"
        open(csvfile, "w").write(csv_body)


def sec2_parse_602(data, meta):
    ...
    if data.tell() + 3 < file_size:
        sec3_magic, _ = struct.unpack(
            ">HH", data.read(4)
        )  # _ is sec3_ver, though it is not needed and unused.
        ...
        if sec3_magic == 0xACED:
            ...
            if sec3_type == 0x77:  # Data block
                ...
                if block_raw[0:3] == b"\x1F\x8B\x08":  # Valid GZIP header
                    ...
                    sec3_unc = zlib.decompress(
                        block_raw, 15 + 32
                    )  # Trick to force bitwindow size
                    print(sec3_unc)
            else:
                print("Unknown serialization opcode found")


def sec3_parse(data, meta):
    """Parse Section three of the file.

    Section three contains a copy of the JAR manifest data.
    """
    data.seek(128 + meta["sec2_len"])
    sec3_data = data.read(meta["sec3_len"])
...
def sec4_parse(data, meta):
    ...
    file_size = meta["filesize"]
    unknowns = 0
    data.seek(128 + meta["sec2_len"] + meta["sec3_len"])
    sec4_magic, _ = struct.unpack(
        ">HH", data.read(4)
    )  # _ is sec4_ver, though it is not needed and unused.
    if (
        sec4_magic == 0xACED
    ):  # Magic number for Java serialized data, version always appears to be 5
        while (
            not data.tell() == file_size
        ):  # If current offset isn't at end of file yet
            if unknowns > 5:
...
def main():
    ...
    arg_parse = argparse.ArgumentParser(
        description=f"Java IDX Parser v{__VERSION__} -- by @bbaskin and digitalsleuth"
    )
    arg_parse.add_argument(
        "-c", "--csv", help="Generates pipe (|) separated file of results (to avoid comma collision)", action="store_true"
    )
    arg_parse.add_argument("file", metavar="FILENAME", help="IDX file to process")
    ...
    if os.path.isfile(args.file):
        try:
            ...
            print(f"Unable to open {args.file}: {err}")
            sys.exit(1)
    ...
    if cache_ver not in (602, 603, 604, 605, 606):
        ...
        sys.exit()
    print(
        f"IDX file: {args.file} (IDX File Version {cache_ver / 100}.{cache_ver - 600:02})"
    )
    ...
    if cache_ver in {602, 603, 604, 605}:
        ...
        print(
            f"Last modified date: {time.strftime('%a, %d %b %Y %X GMT', time.gmtime(last_modified_date))} (epoch: {last_modified_date})"
        )
        if expiration_date:
            print(
                f"Expiration date: {time.strftime('%a, %d %b %Y %X GMT', time.gmtime(expiration_date))} (epoch: {expiration_date})"
            )
        if validation_date:
            print(
                f"Validation date: {time.strftime('%a, %d %b %Y %X GMT', time.gmtime(validation_date))} (epoch: {validation_date})"
            )
        ...
        if cache_ver == 602:
            ...
            if blacklist_timestamp:
                print(
                    f"Blacklist Expiration date: {time.strftime('%a, %d %b %Y %X GMT', time.gmtime(blacklist_timestamp))} (epoch: {blacklist_timestamp})"
                )
            if cert_expiration_date:
                print(
                    f"Certificate Expiration date: {time.strftime('%a, %d %b %Y %X GMT', time.gmtime(cert_expiration_date))} (epoch: {cert_expiration_date})"
                )
    else:
    ...
    if sec2_len:
        if cache_ver == 602:
            sec2_parse_602(data, meta)
        else:
            sec2_parse(data, meta)

    if sec3_len:
        print("\n[*] Section 3 (Jar Manifest) found:")
        sec3_parse(data, meta)

    if sec4_len:
        print("\n[*] Section 4 (Code Signer) found:")
        sec4_parse(data, meta)
    ...
    if args.csv:
        print(f"\n[*] CSV file written to {meta['csvname']}")
